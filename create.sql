...
create table Badge (badgeID int identity not null, active BIT DEFAULT 1 not null, badgeName varchar(255) not null, benefit double precision, description varchar(255) not null, primary key (badgeID));
create table Booking (bookingID bigint identity not null, apartment varchar(255) not null, customersRating int, customersReview NVARCHAR(300) NULL, date datetime2, providersRating int, providersReview NVARCHAR(300) NULL, status int not null, total bigint, unitPrice bigint not null, buildingID int not null, customerID bigint not null, proServiceID bigint not null, primary key (bookingID));
...
create table Building (buildingID int identity not null, active BIT DEFAULT 1 not null, buildingName varchar(255) not null, note varchar(255), primary key (buildingID));
...
create table Customer (customerID bigint identity not null, active BIT DEFAULT 1 not null, address varchar(255) not null, avatar NVARCHAR(300) DEFAULT 'https://media.istockphoto.com/id/1311497219/vector/stylish-avatar-in-black-in-a-yellow-circle-profile-template-for-websites-applications-social.jpg?s=612x612&w=0&k=20&c=SzDRaxQ_ckdqqPbjFqw23uTgj6bbu3x4G66O7j08o6U=', createDate datetime2 not null, dob datetime2, email varchar(255) not null, fullName varchar(255) not null, gender bit not null, password varchar(255) not null, phone varchar(255) not null, rating double precision, role NVARCHAR(10) DEFAULT 'customer' not null, username varchar(255) not null, buildingID int not null, primary key (customerID));
...
create table Provider (providerID bigint identity not null, active BIT DEFAULT 1 not null, address varchar(255) not null, avatar NVARCHAR(300) DEFAULT 'https://media.istockphoto.com/id/1311497219/vector/stylish-avatar-in-black-in-a-yellow-circle-profile-template-for-websites-applications-social.jpg?s=612x612&w=0&k=20&c=SzDRaxQ_ckdqqPbjFqw23uTgj6bbu3x4G66O7j08o6U=', createDate datetime2 not null, email varchar(255) not null, fullName varchar(255) not null, gender bit not null, password varchar(255) not null, phone varchar(255) not null, rating double precision, role NVARCHAR(10) DEFAULT 'provider' not null, username varchar(255) not null, badgeID int not null, buildingID int not null, primary key (providerID));
...
create table ServiceCategory (categoryID int identity not null, active BIT DEFAULT 1 not null, categoryName varchar(255) not null, description varchar(255) not null, primary key (categoryID));
...
create table SubscriptionPlan (planID int identity not null, active BIT DEFAULT 1 not null, description varchar(255) not null, duration int not null, price bigint not null, primary key (planID));
