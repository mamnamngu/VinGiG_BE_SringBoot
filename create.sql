...
create table Booking (bookingID bigint identity not null, apartment varchar(255) not null, customersRating int, customersReview NVARCHAR(500) NULL, date datetime2, providersRating int, providersReview NVARCHAR(500) NULL, status int not null, total bigint, unitPrice bigint not null, buildingID int not null, customerID bigint not null, proServiceID bigint not null, primary key (bookingID));
...
create table Building (buildingID int identity not null, active BIT DEFAULT 1 not null, buildingName varchar(255) not null, note nvarchar(255), primary key (buildingID));
...
create table GiGService (serviceID int identity not null, active BIT DEFAULT 1 not null, description NVARCHAR(300) not null, fee bigint not null, priceMax bigint not null, priceMin bigint not null, serviceName NVARCHAR(200) not null, unit NVARCHAR(20) not null, categoryID int not null, primary key (serviceID));
