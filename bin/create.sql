...
create table GiGService (serviceID int identity not null, description NVARCHAR(300) not null, fee bigint not null, priceMax bigint not null, priceMin bigint not null, serviceName NVARCHAR(100) not null, unit NVARCHAR(20) not null, categoryID int not null, primary key (serviceID));
...
create table Provider (providerID bigint identity not null, address varchar(255) not null, avatar NVARCHAR(150) DEFAULT 'https://media.istockphoto.com/id/1311497219/vector/stylish-avatar-in-black-in-a-yellow-circle-profile-template-for-websites-applications-social.jpg?s=612x612&w=0&k=20&c=SzDRaxQ_ckdqqPbjFqw23uTgj6bbu3x4G66O7j08o6U=', email varchar(255) not null, fullName varchar(255) not null, gender bit not null, password varchar(255) not null, phone varchar(255) not null, rating double precision, username varchar(255) not null, badgeID int not null, buildingID int not null, primary key (providerID));
...
create table SubscriptionPlan (planID int identity not null, description varchar(255) not null, duration int not null, price bigint not null, primary key (planID));
